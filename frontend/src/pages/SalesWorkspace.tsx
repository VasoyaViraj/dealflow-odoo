...
/**
 * Creates the customer account and its portal login in one step — the backend
 * writes both `customers` and `users` in a single transaction and links them,
 * since there is no separate screen for wiring the two together.
 */
function CreateCustomerModal({
  ...
  const create = async () => {
    ...
    try {
      ...
      onCreated(readCustomerResult(r.data));
    } catch (e) {
      showToast(errorMessage(e, 'Failed to create customer'), 'error');
    } finally { setLoading(false); }
  };

  const field = 'w-full bg-soft border border-hairline rounded-lg px-3 py-2.5 text-sm text-ink focus:outline-none focus:border-ink transition';
  const label = 'block text-sm text-subtle mb-1.5 font-medium';

  return (
    <>
      <div className="fixed inset-0 bg-ink/40 backdrop-blur-sm z-40" onClick={onClose} />
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto">
        <div className="bg-canvas border border-hairline rounded-lg p-6 w-full max-w-lg shadow-lg my-8">
          <div className="flex items-center justify-between mb-5">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input value={form.name} onChange={set('name')} className={field} placeholder="Acme Corp" />
            </div>

            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className={label}>Contact First Name</label>
                <input value={form.contactFirstName} onChange={set('contactFirstName')} className={field} placeholder="Jane" />
              </div>
              <div>
                <label className={label}>Contact Last Name</label>
                <input value={form.contactLastName} onChange={set('contactLastName')} className={field} placeholder="Doe" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="email" value={form.email} onChange={set('email')} className={field} placeholder="contact@acme.com" />
              </div>
              <div>
                <label className={label}>Phone</label>
                <input type="tel" value={form.phone} onChange={set('phone')} className={field} placeholder="+1 555-0123" />
              </div>
            </div>

            <div>
              <label className={label}>Address</label>
              <input value={form.address} onChange={set('address')} className={field} placeholder="1 Market St, San Francisco, CA" />
            </div>

            <div>
              <label className={label}>Tier</label>
              <select value={form.tier} onChange={set('tier')} className={field}>
            ...
            <div className="border-t border-hairline pt-4 space-y-3">
              <label className="flex items-center gap-2 text-sm text-ink cursor-pointer">
                <input type="checkbox" checked={createPortalUser} onChange={e => setCreatePortalUser(e.target.checked)} className="accent-current" />
                Create a portal login for this customer
              </label>

              {createPortalUser && (
                <>
                  <label className="flex items-center gap-2 text-sm text-subtle cursor-pointer">
                    <input type="checkbox" checked={setOwnPassword} onChange={e => setSetOwnPassword(e.target.checked)} className="accent-current" />
                  ...
                  <p className="text-xs text-subtle flex items-center gap-1.5 bg-soft p-2 rounded">
                    <Info size={12} />
                    {setOwnPassword
                      ? 'The login uses the email above.'
                      : 'A password is generated and shown once after saving.'}
                  </p>
                </>
              )}
            </div>
...
export default function SalesWorkspace() {
  ...
  const [createdCustomer, setCreatedCustomer] = useState<CustomerCreateResult | null>(null);
  ...
  return (
    <AppShell>
      ...
      {createdCustomer && (
        <PortalCredentialsDialog result={createdCustomer} onClose={() => setCreatedCustomer(null)} />
      )}
