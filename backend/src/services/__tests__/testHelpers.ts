...
export async function createQuotation(
  ...
  for (const line of lines) {
    const discountAmount = (parseFloat(line.unitPrice) * line.quantity * parseFloat(line.discountPercent) / 100).toFixed(2);
    const finalPrice = (parseFloat(line.unitPrice) * line.quantity - parseFloat(discountAmount)).toFixed(2);
