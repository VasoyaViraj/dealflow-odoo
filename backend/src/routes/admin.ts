...
/**
 * Creating a customer also provisions (or links) its CUSTOMER-role portal user
 * — see services/customers/customerService.ts. Updates stay column-only.
 */
...
router.get('/customers', ...adminOnly, async (_req, res) => {
  try {
    // Left join so the table can show whether a portal login exists at all —
    // there was previously no way to see the link from any screen.
    const rows = await db
      .select({
        id: customers.id,
        name: customers.name,
      ...
      .from(customers)
      .leftJoin(users, eq(customers.linkedUserId, users.id))
...
router.post('/customers', ...adminOnly, async (req, res) => {
  try {
    ...
    if (!parsed.success) return res.status(400).json({ success: false, error: formatZodError(parsed.error) });

    const result = await createCustomerWithPortalUser(parsed.data);

    await logAudit(req.user!.id, 'CUSTOMER_CREATED', 'CUSTOMER', result.customer.id, {
...
/**
 * Repair path for customers that have no portal login — the ones seeded or
 * created before creation provisioned users. Creates the user, or adopts an
 * existing unlinked CUSTOMER user with that email.
 */
router.post('/customers/:id/portal-user', ...adminOnly, async (req, res) => {
  try {
    const parsed = attachPortalUserSchema.safeParse(req.body ?? {});
    if (!parsed.success) return res.status(400).json({ success: false, error: formatZodError(parsed.error) });

    const result = await attachPortalUser(String(req.params.id), parsed.data);

    await logAudit(req.user!.id, 'CUSTOMER_PORTAL_USER_LINKED', 'CUSTOMER', result.customer.id, {
      portalUserId: result.portalUser?.id ?? null,
      linkage: result.linkage,
    });
