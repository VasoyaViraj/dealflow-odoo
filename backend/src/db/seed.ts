...
async function seed() {
  ...
  if (existingQuotation) {
    ...
    async function seedQuotation(
      label: string,
      lines: Array<{ productId: string; quantity: number; unitPrice: string; costPrice: string; discountPercent: string }>,
    ) {
      let subtotal = 0;
    ...
    if (acmeId && laptopId && setupServiceId && salesRepId) {
      await seedQuotation('DEMO_SCRIPT Step 2-8 (Laptop x2@12% + Setup Service x1@18%, expect riskScore 13.79 -> SALES_MANAGER)', [
        { productId: laptopId, quantity: 2, unitPrice: '1200.00', costPrice: '800.00', discountPercent: '12' },
        { productId: setupServiceId, quantity: 1, unitPrice: '500.00', costPrice: '100.00', discountPercent: '18' },
      ]);

      await seedQuotation('DEMO_SCRIPT Step 9 (Setup Service x10@25%, expect riskScore 150 -> FINANCE)', [
        { productId: setupServiceId, quantity: 10, unitPrice: '500.00', costPrice: '100.00', discountPercent: '25' },
      ]);
