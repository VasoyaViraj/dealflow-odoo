  ...
  return (
    <div className="rounded-xl border border-violet-500/30 bg-violet-500/5 p-5">
      <div className="flex items-center justify-between mb-4">
        <div>
          <p className="text-sm font-semibold text-violet-200">Manual split</p>
        ...
        <button
          onClick={onCancel}
          className="w-7 h-7 rounded-lg flex items-center justify-center text-zinc-500 hover:text-zinc-200 hover:bg-zinc-800 transition"
        >
          <X size={14} />
        </button>
      </div>
      ...
      <div className="space-y-4">
        {demandLines.map((line) => {
          ...
          return (
            <div key={line.quotationLineId} className="rounded-lg border border-zinc-800 bg-zinc-900/60 p-4">
              <div className="flex items-baseline justify-between mb-3">
                <p className="text-sm font-medium text-zinc-100">{line.productName}</p>
                <p className={`text-xs font-medium ${allocated > line.quantity ? 'text-red-400' : short > 0 ? 'text-amber-400' : 'text-emerald-400'}`}>
              ...
              {sources.length === 0 ? (
                <p className="text-xs text-zinc-500">No warehouse currently holds this product.</p>
              ) : (
                    ...
                    return (
                      <label
                        ...
                        className={`flex items-center justify-between gap-3 rounded-lg border px-3 py-2 ${
                          over ? 'border-red-500/40 bg-red-500/5' : 'border-zinc-700 bg-zinc-800/60'
                        }`}
                      >
                        <span className="min-w-0">
                          <span className="block text-xs font-medium text-zinc-200 truncate">{s.warehouseName}</span>
                        ...
                        <input
                          ...
                          className="w-16 shrink-0 bg-zinc-900 border border-zinc-700 rounded-lg px-2 py-1.5 text-sm text-right text-zinc-100 focus:outline-none focus:border-violet-500 transition"
                        />
                      </label>
                    );
                  })}
      ...
      {(overAllocated.length > 0 || overStock.length > 0) && (
        <div className="mt-4 flex items-start gap-2 text-xs px-3 py-2.5 rounded-lg border bg-red-500/10 border-red-500/30 text-red-300">
      ...
      <div className="flex items-center justify-end gap-2 mt-5">
        <button
          onClick={onCancel}
          className="px-4 py-2 rounded-lg text-sm text-zinc-400 hover:text-zinc-100 hover:bg-zinc-800 transition"
        >
          Cancel
        </button>
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold bg-violet-600 hover:bg-violet-500 text-white transition disabled:opacity-40 disabled:cursor-not-allowed"
        >
